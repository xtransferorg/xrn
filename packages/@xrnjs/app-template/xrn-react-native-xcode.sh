...
echo "XT_DEST 路径：$XT_DEST"
XT_PROJECT_ROOT="${SRCROOT%/*}"
echo "XT_PROJECT_ROOT 路径：$XT_PROJECT_ROOT"

XT_MAIN_BUNDLE_SOURCE="${XT_PROJECT_ROOT}/release_ios"
echo "XT_MAIN_BUNDLE_SOURCE 路径：$XT_MAIN_BUNDLE_SOURCE"
XT_COMMON_BUNDLE_SOURCE=$XT_MAIN_BUNDLE_SOURCE
echo "XT_COMMON_BUNDLE_SOURCE 路径：$XT_COMMON_BUNDLE_SOURCE"
XT_DIFF_PATCH_SOURCE="${XT_PROJECT_ROOT}/HDiffPatch"
echo "XT_DIFF_PATCH_SOURCE 路径：$XT_DIFF_PATCH_SOURCE"
XT_DIFF_PATCH_DEST="${XT_DEST}/HDiffPatch"
echo "XT_DIFF_PATCH_DEST 路径：$XT_DIFF_PATCH_DEST"
...
echo "XT_DIFF_PATCH_SOURCE 路径：$XT_DIFF_PATCH_SOURCE 不存在"
...
if [[ -d "$XT_MAIN_BUNDLE_SOURCE/assets" ]] && find "$XT_MAIN_BUNDLE_SOURCE" -type f -name "*.jsbundle" | grep -q .; then
	cp -R "$XT_MAIN_BUNDLE_SOURCE/assets" $XT_DEST
...
echo "XT_PlistBuddy_CLI: $XT_PlistBuddy_CLI"
XT_INFO_PLIST_PATH="${SRCROOT}/${INFOPLIST_FILE}"
echo "XT_INFO_PLIST_PATH 路径: $XT_INFO_PLIST_PATH"
XT_SUB_BUNDLE_PLIST_PATH="${SRCROOT}/BundleEventCenter/bundleController/xtBundles.plist"
echo "XT_SUB_BUNDLE_PLIST_PATH 路径：$XT_SUB_BUNDLE_PLIST_PATH"
...
xt_check_bundle_existence_and_copy() {
	...
	echo "参数1：$xt_bundle_source"
	echo "参数2：$xt_bundle_name"
...
echo "XT_MAIN_BUNDLE_NAME：$XT_MAIN_BUNDLE_NAME"
...
echo "XT_COMMON_BUNDLE_NAME 值：$XT_COMMON_BUNDLE_NAME"
...
echo "xt_sub_bundle_array_keys 值：$xt_sub_bundle_array_keys"
...
do
	...
	echo "xt_sub_bundle_name 子bundle 名称：$xt_sub_bundle_name"
