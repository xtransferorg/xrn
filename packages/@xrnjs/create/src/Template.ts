...
export async function extractAndPrepareTemplateAppAsync(
  ...
  debug(
    `Extracting template app (pkg: ${npmPackage}, projectName: ${projectName})`,
  );

  const packageName = npmPackage || "@xrnjs/app-template";
  const { type, uri } = resolvePackageModuleId(packageName);
  ...
  if (type === "repository") {
    ...
    const resolvedUri =
      type === "file" ? uri : getResolvedTemplateName(applyBetaTag(uri));
...
export async function sanitizeTemplateAsync(projectRoot: string) {
  ...
  const ignoreTemplatePath = path.join(__dirname, "../template/gitignore");
  const ignorePath = path.join(projectRoot, ".gitignore");
  if (!fs.existsSync(ignorePath)) {
    await fs.promises.copyFile(ignoreTemplatePath, ignorePath);
  }

  const npmrcTemplatePath = path.join(__dirname, "../template/npmrc");
  const npmrcPath = path.join(projectRoot, ".npmrc");

  if (!fs.existsSync(npmrcPath)) {
    await fs.promises.copyFile(npmrcTemplatePath, npmrcPath);
  }
  ...
  const appContent = (await appFile.readAsync()) as
    | ExpoConfig
    | Record<"expo", ExpoConfig>;
  ...
  const name = applyKnownNpmPackageNameRules(inputName) || "app";
  ...
  fs.promises.unlink($packageJsonPath).catch(() => {});
}
...
export function logProjectReady({
  ...
  if (cdPath) {
    ...
    console.log(
      `To run your project, run one of the following ${packageManager} commands.`,
    );
    console.log();
  }

  // console.log(`- ${chalk.bold(formatRunCommand(packageManager, "start"))}`);
}
...
export function logNewSection(title: string) {
  const disabled = env.CI || env.XRN_DEBUG;
  const spinner = ora({
