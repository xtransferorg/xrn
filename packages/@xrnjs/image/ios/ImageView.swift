...
public final class ImageView: RCTView {
  ...
  var contentFit: ExpoImageContentFit = .cover
  ...
  var cachePolicy: ExpoImageCachePolicy = .disk
  ...
  @objc func setPriority(_ newPriority: ExpoImagePriority) {
    ...
    if let priority = newPriority.toSDWebImageOptions() {
      loadingOptions.insert(priority)
    }
    reload()
  }
  ...
  private func imageLoadCompleted(
    ...
    guard finished else {
      print("[Expo Image] Error Loading the image has been canceled")
      return
    }
    
    // svg image and exist not support tag
    if let data = data,
        SDImageSVGCoder.shared.canDecode(from: data) &&
        SDImageSVGCoder.containsNoSupportTag(in: data) {
      let errorDic: [String : Any] = ["isSvg": true, "data": String(data: data, encoding: .utf8) ?? ""]
      
      if let jsonData = try? JSONSerialization.data(withJSONObject: errorDic),
         let jsonString = String(data: jsonData, encoding: .utf8) {
        onError?(["error": jsonString])
      } else {
        onError?(["error": "svg image contain no support tag, but data parse fail!"])
      }
      return
    }
   ...
   */
  var placeholderContentFit: ExpoImageContentFit = .scaleDown
  ...
  private func applyContentPosition(contentSize: CGSize, containerSize: CGSize) {
    let offset = contentPosition.offset(withContentSize: contentSize, containerSize: containerSize)
  ...
  private func setImage(_ image: UIImage?, contentFit: ExpoImageContentFit, isPlaceholder: Bool) {
    sdImageView.contentMode = contentFit.toContentMode()
