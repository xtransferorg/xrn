...
exports.syncAllDependenciesAcrossWorkspaces = exports.syncDependencyVersionAcrossWorkspaces = void 0;
 ...
 * @returns 是否有修改
 */
const updateDependenciesWithTargetVersion = (pkgData, getTargetVersion, pkgName) => {
    let modified = false;
        ...
        if (pkgData[field]) {
            Object.keys(pkgData[field]).forEach((dep) => {
                ...
                if (targetVersion && pkgData[field][dep] !== targetVersion) {
                    console.log(`更新 ${pkgName} 中的 ${dep} 依赖版本: ${pkgData[field][dep]} -> ${targetVersion}`);
 ...
 */
const syncDependencyVersionAcrossWorkspaces = async (targetDepName) => {
    const _packages = await (0, utils_1.getWorkspacePackages)();
    const packages = _packages.filter((pkg) => !SKIP_PACKAGES.includes(pkg.name));
    const targetDepPackageInfo = await (0, utils_1.findWorkspacePackage)(targetDepName);
    if (!targetDepPackageInfo) {
        console.error(`未找到依赖包 ${targetDepName}`);
        return;
    }
    // 获取目标依赖包的版本（取自 node_modules 中的安装版本）
    const targetDepPackageJsonPath = path.join(Constants_1.XRN_DIR, targetDepPackageInfo.location, "package.json");
    ...
    // 定义获取目标依赖版本的函数
    const getTargetVersion = (depName) => {
    ...
    packages.forEach((pkg) => {
        const packageJsonPath = path.join(Constants_1.XRN_DIR, pkg.location, "package.json");
        if (!fs.existsSync(packageJsonPath))
            return;
        const pkgData = readJSON(packageJsonPath);
        const modified = updateDependenciesWithTargetVersion(pkgData, getTargetVersion, pkg.name);
        if (modified) {
            writeJSON(packageJsonPath, pkgData);
        }
    });
    console.log(`所有工作区中对 ${targetDepName} 的依赖已更新为版本 ${targetDepVersion}`);
    // 新增：同步 apps/*/package.json 里的 targetDepName 依赖
    const appsDir = path.join(Constants_1.XRN_DIR, "apps");
    ...
    appDirs.forEach((appName) => {
        ...
        const modified = updateDependenciesWithTargetVersion(pkgData, (depName) => (depName === targetDepName ? targetDepVersion : undefined), pkgData.name);
        if (modified) {
...
exports.syncDependencyVersionAcrossWorkspaces = syncDependencyVersionAcrossWorkspaces;
/**
 * 同步工作区中所有包的依赖版本
 * 如果依赖项在工作区中存在，则更新为工作区中的版本
 */
const syncAllDependenciesAcrossWorkspaces = async () => {
    const _packages = await (0, utils_1.getWorkspacePackages)();
    const packages = _packages.filter((pkg) => !SKIP_PACKAGES.includes(pkg.name));
    ...
    appDirs.forEach((appName) => {
        ...
        if (!fs.existsSync(appPkgJson))
            return;
        const pkgData = readJSON(appPkgJson);
        const modified = updateDependenciesWithTargetVersion(pkgData, (depName) => workspaceVersions[depName], pkgData.name);
        if (modified) {
            writeJSON(appPkgJson, pkgData);
...
exports.syncAllDependenciesAcrossWorkspaces = syncAllDependenciesAcrossWorkspaces;
//# sourceMappingURL=syncDeps.js.map