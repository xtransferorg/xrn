...
describe('static resource links', () => {
  const file = makeFile('index.mdx');

  it('rewrites /static/ to /xrn/static/', () => {
    expect(rewrite(file, '/static/images/get-started/android-device.png')).toBe('/xrn/static/images/get-started/android-device.png');
  });

  it('rewrites /static/ with query parameters', () => {
    expect(rewrite(file, '/static/images/logo.png?v=123')).toBe('/xrn/static/images/logo.png?v=123');
  });

  it('does not rewrite external static links', () => {
    expect(rewrite(file, 'https://example.com/static/image.png')).toBe('https://example.com/static/image.png');
  });
});
